...
import type { AnomalyEnvelope } from '../../api/contracts'
...
export function CostCourtroom({ open, data, error, loading, snapshotId, onClose, onRetry }: { open: boolean; data?: AnomalyEnvelope; error: unknown; loading: boolean; snapshotId?: string; onClose: () => void; onRetry: () => void }) {
  ...
  return <Drawer open={open} onOpenChange={(next) => { if (!next) onClose() }} title="Cost Courtroom" description="Charge, evidence, and verdict from the active trusted snapshot">{loading ? <LoadingSkeleton label="Loading Cost Courtroom" /> : error ? <ErrorState error={error} onRetry={onRetry} /> : anomaly ? <>
